...
class live_complete_Post_Related
{
    /**
    ...
    public function __construct()
    {
        ...
        if (!is_admin()) {
            ...
            // add_action( 'live_complete_site_content_type', array( $this,'site_loop_heading' ), 21 ); 
            add_action('live_complete_site_content_type', array($this, 'site_content_type'), 30);
        }

        add_action('live_complete_posts_blog_media', array($this, 'render_thumbnail'));


        add_action('live_complete_loop_navigation', array($this, 'site_loop_navigation'));
    ...
    function site_content_type()
    {
        ...
        $type = apply_filters('live_complete_content_type_filter', live_complete_get_option('blog_loop_content_type'));
    ...
    public function content_read_more_link($more)
    {
        ...
        return sprintf(
            ...
            esc_html(live_complete_get_option('read_more_text'))
        );
    }
    ...
    function site_loop_navigation($type = '')
    {
        ...
        if ($type == '') {
            $type = apply_filters('live_complete_loop_navigation_filter', get_theme_mod('live_complete_loop_navigation', 'default'));
        }
        ...
        if ($type == 'default'):
            ...
            the_posts_navigation(
                array(
                    'prev_text' => '<span class="btn-wrap">' . esc_html__('Previous Posts', 'live-complete') . '</span><span class="icon"><i class="icofont-rounded-double-left"></i></span>',
                    'next_text' => '<span class="btn-wrap">' . esc_html__('Next Posts', 'live-complete') . '</span><span class="icon"><i class="icofont-rounded-double-right"></i></span>',
                    'screen_reader_text' => __('Posts navigation', 'live-complete')
                )
            );
            ...
            the_posts_pagination(array(
                ...
                'type' => 'list',
                'mid_size' => 2,
                'prev_text' => esc_html__('Previous', 'live-complete'),
                'next_text' => esc_html__('Next', 'live-complete'),
                'screen_reader_text' => esc_html__('&nbsp;', 'live-complete'),
            ));
            echo '</div>';
        endif;
    }
...
$live_complete_post_related = new live_complete_Post_Related();
