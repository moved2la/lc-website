...
class live_complete_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'live_complete_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'live_complete_site_content_type', array( $this, 'render_meta_list' ), 20, 1 );
		add_action( 'live_complete_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'live-complete' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'live-complete' ) . ':</span>';
		foreach ( $tags as $tag ) {
...
$live_complete_post_meta = new live_complete_Post_Meta();