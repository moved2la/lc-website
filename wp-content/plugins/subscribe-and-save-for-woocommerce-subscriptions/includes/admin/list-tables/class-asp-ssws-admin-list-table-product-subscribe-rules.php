...
/**
 * Admin product subscribe rules handler.
 * 
 * @class ASP_SSWS_Admin_List_Table_Product_Subscribe_Rules
 * @package Class
 */
class ASP_SSWS_Admin_List_Table_Product_Subscribe_Rules extends WC_Admin_List_Table {
	 ...
	 * @var string
	 */
	protected $list_table_type = 'asp_prod_subs_rule';
	...
	protected function render_blank_state() {
		...
		echo '<h2 class="woocommerce-BlankState-message">' . esc_html__( 'Product subscribe rules are a way to create flexible product level subscribe configuration for your customers. They will appear here once created.', 'subscribe-and-save-for-woocommerce-subscriptions' ) . '</h2>';
		...
		echo '<a class="woocommerce-BlankState-cta button-primary button ' . ( $has_one_plan ? '' : 'asp-ssws-not-clickable' ) . '" href="' . esc_url( admin_url( 'post-new.php?post_type=asp_prod_subs_rule' ) ) . '">' . esc_html__( 'Create your first product subscribe rule', 'subscribe-and-save-for-woocommerce-subscriptions' ) . '</a>';
		echo '</div>';
	}
	...
	public function render_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'name':
				printf( '<b><a href="%1$s">%2$s</a></b>', esc_url( $subscribe_rule->get_edit_subscribe_rule_url() ), wp_kses_post( $subscribe_rule->get_name() ) );
				break;
			case 'status':
...
new ASP_SSWS_Admin_List_Table_Product_Subscribe_Rules();
