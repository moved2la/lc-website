...
add_action('live_complete_add_to_cart_icon', 'woocommerce_template_loop_add_to_cart', 10);
 ...
 * @return void
 */
function live_complete_woocommerce_setup()
{
    add_theme_support(
        'woocommerce',
        array(
    ...
    register_sidebar(array(
        'name'          => esc_html__('Woocommerce Sidebar', 'live-complete'),
...
add_action('after_setup_theme', 'live_complete_woocommerce_setup');
...
function live_complete_woocommerce_scripts()
{
    ...
    wp_add_inline_style('live-complete-woocommerce-style', $inline_font);

    wp_enqueue_script('live-complete-woocommerce', get_theme_file_uri('/assets/js/live-complete-woocommerce.js'), 0, '1.1', true);
}
add_action('wp_enqueue_scripts', 'live_complete_woocommerce_scripts');
 ...
 */
function live_complete_woocommerce_active_body_class($classes)
{
    $classes[] = 'woocommerce-active';

    return $classes;
}
add_filter('body_class', 'live_complete_woocommerce_active_body_class');
 ...
 */
function live_complete_woocommerce_related_products_args($args)
{
    $defaults = array(
        'posts_per_page' => 3,
        'columns'        => 3,
...
add_filter('woocommerce_output_related_products_args', 'live_complete_woocommerce_related_products_args');
add_filter('woocommerce_upsell_display_args', 'live_complete_woocommerce_related_products_args');

add_filter('woocommerce_cross_sells_columns', 'live_complete_change_cross_sells_columns');

function live_complete_change_cross_sells_columns($columns)
{
    return 3;
}
/**
...
if (! function_exists('live_complete_woocommerce_wrapper_before')) {
    /**
     * Before Content.
     *
     ...
     * @return void
     */
    function live_complete_woocommerce_wrapper_before()
    {
        /**
...
add_action('woocommerce_before_main_content', 'live_complete_woocommerce_wrapper_before');

if (! function_exists('live_complete_woocommerce_wrapper_after')) {
    /**
     * After Content.
     *
     ...
     * @return void
     */
    function live_complete_woocommerce_wrapper_after()
    {
        /**
...
add_action('woocommerce_after_main_content', 'live_complete_woocommerce_wrapper_after');
 ...
 *
	<?php
		if ( function_exists( 'live_complete_woocommerce_header_cart' ) ) {
			live_complete_woocommerce_header_cart();
		}
	?>
 */

if (! function_exists('live_complete_woocommerce_cart_link_fragment')) {
    /**
     * Cart Fragments.
     *
     ...
     */
    function live_complete_woocommerce_cart_link_fragment($fragments)
    {
        ob_start();
        live_complete_woocommerce_cart_link();
        $fragments['a.cart-contents'] = ob_get_clean();

        return $fragments;
    }
}
add_filter('woocommerce_add_to_cart_fragments', 'live_complete_woocommerce_cart_link_fragment');

if (! function_exists('live_complete_woocommerce_cart_link')) {
    /**
     * Cart Link.
     *
     ...
     * @return void
     */
    function live_complete_woocommerce_cart_link()
    {
?>
        <?php if (class_exists('ATA_WC_Smart_Popup_Cart') || class_exists('ATA_WC_Smart_Popup_Cart_PRO')) : ?>
            <a class="cart-contents single_add_to_cart_flyer" href="javascript:void(0)" title="<?php esc_attr_e('View your shopping cart', 'live-complete'); ?>">
            <?php else : ?>
                <a class="cart-contents single_add_to_cart_flyer" href="<?php echo esc_url(wc_get_cart_url()); ?>" title="<?php esc_attr_e('View your shopping cart', 'live-complete'); ?>">
                <?php
            endif;
            $item_count_text = sprintf(
                /* translators: number of items in the mini cart. */
                _n('%d', '%d', WC()->cart->get_cart_contents_count(), 'live-complete'),
    ...
    if (! function_exists('live_complete_woocommerce_header_cart')) {
         ...
         * @return void
         */
        function live_complete_woocommerce_header_cart()
        {
            if (is_cart()) {
            ...
            ?>
                <ul id="site-header-cart" class="site-header-cart">
                    <li class="<?php echo esc_attr($class); ?>">
                        <?php live_complete_woocommerce_cart_link(); ?>
                    </li>
                    <li>
                        <?php
                        $instance = array(
    ...
    if (! function_exists('live_complete_toolbar_start')) {
        /**
         * Insert the opening anchor tag for products in the loop.
         */
        function live_complete_toolbar_start()
        {
            echo '<div class="live-complete-toolbar clearfix">';
        }

        add_action('woocommerce_before_shop_loop', 'live_complete_toolbar_start', 20);
    }

    /**
     * Add Custom Result Counter.
     */
    function live_complete_result_count()
    {
    ...
    add_action('woocommerce_before_shop_loop', 'live_complete_product_filter_button', 29);
    ...
    if (! function_exists('live_complete_header_toolbar_end')) {
         ...
         * Insert the opening anchor tag for products in the loop.
         */
        function live_complete_header_toolbar_end()
        {
            echo '<div class="clearfix"></div></div>';
        }

        add_action('woocommerce_before_shop_loop', 'live_complete_header_toolbar_end', 30);
    }


    if (! function_exists('live_complete_loop_shop_per_page')) :
        /**
         * Returns correct posts per page for the shop
         *
         * @since 1.0.0
         */
        function live_complete_loop_shop_per_page()
        {
        ...
        add_filter('loop_shop_per_page', 'live_complete_loop_shop_per_page', 20);
    endif;
     ...
     */
    function live_complete_woocommerce_loop_columns()
    {
        return 3;
    }
    add_filter('loop_shop_columns', 'live_complete_woocommerce_loop_columns');



    if (! function_exists('live_complete_loop_product_thumbnail')) {

        /**
         * Get the product thumbnail for the loop.
         */
        function live_complete_loop_product_thumbnail()
        {
            global $product;
        ...
        add_action('woocommerce_before_shop_loop_item_title', 'live_complete_loop_product_thumbnail', 30);

        add_action('woocommerce_before_shop_loop_item_title', 'woocommerce_template_loop_product_link_close', 99);
    }


    if (! function_exists('live_complete_loop_content_before')) {

        /**
         * end the product content wrap
         */
        function live_complete_loop_content_before()
        {
            echo '<div class="product_wrap">';
        }
        add_action('woocommerce_shop_loop_item_title', 'live_complete_loop_content_before', 5);
    }


    if (! function_exists('live_complete_product_loop_actions')) {

        /**
         * end the product content wrap
         */
        function live_complete_product_loop_actions()
        {
    ...
    if (! function_exists('live_complete_loop_item_title')) {
         ...
         */
        function live_complete_loop_item_title()
        {
        ...
        add_action('woocommerce_shop_loop_item_title', 'live_complete_loop_item_title', 40);
    }


    if (! function_exists('live_complete_loop_content_after')) {

        /**
         * end the product content wrap
         */
        function live_complete_loop_content_after()
        {
            echo '</div>';
        }
        add_action('woocommerce_after_shop_loop_item', 'live_complete_loop_content_after', 999);
    }



    if (! function_exists('live_complete_before_quantity_input_field')) {
        /**
         * before quantity.
         *
         *
         * @return $html
         */
        function live_complete_before_quantity_input_field()
        {
            echo '<button type="button" class="plus"><i class="icofont-plus"></i></button>';
        }
        add_action('woocommerce_before_quantity_input_field', 'live_complete_before_quantity_input_field', 10);
    }

    if (! function_exists('live_complete_after_quantity_input_field')) {
        /**
         * after quantity.
         *
         *
         * @return $html
         */
        function live_complete_after_quantity_input_field()
        {
            echo '<button type="button" class="minus"><i class="icofont-minus"></i></button>';
        }
        add_action('woocommerce_after_quantity_input_field', 'live_complete_after_quantity_input_field', 10);
    }


    function live_complete_product_loop_swatches()
    {
